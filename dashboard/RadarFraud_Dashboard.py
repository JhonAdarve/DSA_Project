...
input_path = os.path.join(project_root, 'data', 'processed_banksim.csv')

df = pd.read_csv(input_path)

metrics_path = os.path.join(project_root, 'data', 'best_model_metrics.json')
...
def calculate_kpis(filtered_df):
    ...
    # Usar métricas del mejor modelo
    recall = best_model_metrics["recall"] * 100  # Asegurar porcentaje
    precision = best_model_metrics["precision"] * 100
    f1_score = best_model_metrics["f1_score"] * 100
    
    # Calcular tiempo promedio de detección
    fraud_cases = filtered_df[filtered_df['fraud'] == 1]
    avg_detection_time = fraud_cases['step'].diff().mean() * 24
    
    # Calcular tendencia del recall
...
def create_correlation_matrix(df):
    # Select only numeric columns, excluding geolocation and time variables
    numeric_df = df.select_dtypes(include=['float64', 'int64']).drop(
        columns=['origin_lat', 'origin_lon', 'merchant_lat', 'merchant_lon', "merchant_address", "origin_neighborhood", "merchant_neighborhood", "weekday", "day", "month", "year", "hour", "date_formatted" ], errors='ignore'
    )
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div([
            # Dinámicas
            html.Div(id='kpi-total-transactions', className='kpi-card'),
            html.Div(id='kpi-total-fraud', className='kpi-card'),
            html.Div(id='kpi-fraud-detection-rate', className='kpi-card'),
            html.Div(id='kpi-avg-detection-time', className='kpi-card'),
...
def update_graphs(start_date, end_date, categories, amount_range):
    ...
    return (
        ...
        create_gender_distribution(filtered_df),
        create_category_distribution(filtered_df),
        create_amount_distribution(filtered_df)
    )

@app.callback(
    [Output('kpi-total-transactions', 'children'),
     Output('kpi-total-fraud', 'children'),
     Output('kpi-fraud-detection-rate', 'children'),
     Output('kpi-avg-detection-time', 'children')],
    [Input('date-filter', 'start_date'),
     Input('date-filter', 'end_date'),
     Input('category-filter', 'value'),
     Input('amount-filter', 'value')]
)
def update_dynamic_kpis(start_date, end_date, categories, amount_range):
    # Aplicar filtros
    filtered_df = df.copy()
    if start_date and end_date:
        filtered_df = filtered_df[
    ...
    total_transactions = len(filtered_df)
    total_fraud = filtered_df['fraud'].sum()
    fraud_detection_rate = (total_fraud / total_transactions) * 100 if total_transactions > 0 else 0
    avg_detection_time = calculate_kpis(filtered_df)['avg_detection_time'] if total_transactions > 0 else 0

    # Crear las tarjetas dinámicas
    return (
        create_kpi_card("Total Transactions", total_transactions, format_type='number'),
        create_kpi_card("Total Fraud Detected", total_fraud, format_type='number'),
        create_kpi_card("Fraud Detection Rate", fraud_detection_rate, target=90, format_type='percentage'),
        create_kpi_card("Avg Detection Time", avg_detection_time, target=1, format_type='time')
    )
